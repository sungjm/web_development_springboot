...
class BlogApiControllerTest {
    ...
    /*
        ...
        : 이 클래스로 만든 자바 객체를 JSON 데이터로 변환하는 것 -> 직렬화
            ...
            형식에 맞게 변환하는 과정이 필요합니다. 이 과정을 직렬화/역직렬화

                1) 직렬화 Java 시스템 내부에서 사용되는 ㄱ댁체를 외부에서 사용하도록
                    변환하는 과정을 뜻하는데,
...
public void addArticle() throws Exception {
    ...
    /*
        ...
        그 이후에 MockMvc를 사요해 HTTP 메서드, URL, 요청본문, 요청 타입 등을 설정 뒤 테스트 요청 보냈습니다.
        contentType() 메서드 : JSON/ XML 등 다양한 타입 중 하나를 선택할 수 있는데, 저희는 JSON 사용했습니다.
        assertThat() 메서드 : 글의 개수가 1인지,
        Article 객체의 field인 title의 값이 "제목"인지
        Article 객체의 field인 content의 값이 "내용"인지
    ...
    public void deleteArticle() throws Exception {
        ...
        // given -2 객체 생성 -> 하나만 생성했다가 지운 다음에 List 전체 불어왔는데 0이면 됩니다.
        Article savedArticle = blogRepository.save(Article.builder()
                .title(title)
                .content(content)
                .build());
        ...
        // then - 검증단계에서 블로그 글 목록을 전부 다 가지고 올겁니다. -> size()채크 했더니 0 이거나 / .isEmpty()
        // 전부 가지고 오는 단계
