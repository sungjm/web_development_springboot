...
/*
    ...
    test -> resources 우클릭하고, insert-member.sql 파일 생성
    작성 후(혹은 확인 후에) 코드 의미:
...
class MemberRepositoryTest {
    ...
    void getAllMembers() {
        ...
        // memberRepository는 상속받은 매서드명을 그대로 가져가니까
    ...
    @Sql("/insert-members.sql")
    @Test
    void getMemberByName() { // MemberRepository.java에는 findByName()이였다는 점에 주목
        // when
    ...
    /*
        이상과 같이 MemberRepository.java에 매서드를 정의하는 것을 '쿼리 매서드'
        일반적인 경우 JPA가 정해준 매서드 이름 규칙을 따르면 쿼리문을 특별히 구현하지 않아도
        매서드처럼 사용할 수 있음(즉 자바만으로 컨트롤이 가능하다는 의미)

        이상의 매서드를 SQL로 작성할 경우
        SELECT * FROM member WHERE name = 'C';

        지금까지 조회 관련 매서드 수업했습니다.
        전체 조회 -> findAll()
        아이디로 조회 -> findById()
        특정 컬럼으로 조회(name) -> 쿼리 매서드 명명 규칙에 맞게 정의 후에 사용(find / get 차이)

        추가 / 삭제 매서드

        INSERT INTO member (id, name) VALUES (1, 'A')라는 쿼리가 있다고 가정했을 때
        JPA에서는 save()라는 매서드를 사용
     */
    @Test       //@Sql애너테이션을 달지 않습니다. 이 매서드에서 1, 'A'에 해당하는 객체를 저장할거라서
    void saveMember() {
        // given
    ...
    /*
        이상의 코드는 하나의 멤버 객체를 추가하는 매서드

        만약 여러 엔티티를 한꺼번에 저장하고 싶다면 saveAll() 매서드를 사용 가능
     */
    @Test
    void saveMembers() {
        ...
        List<Member> members = List.of(new Member(2L,"안근수"),
                new Member(3L,"안근수"));
        // when
        memberRepository.saveAll(members);          //members라는 list에 전체 저장
    ...
    /*
        수정 매서드
        ex) id가 2인 멤버의 이름을 "BC"로 바꾼다고 가정했을 떼,
        ...
        내에서 데이터를 수행해야 합니다.
            -> 매서드만 사용하는 것이 아니라 @Transactional 애너테이션을
                매서드에 추가해야 합니다.
    ...
    /*
        그런제 지금 이상의 매서드에는 @Transactional 애너테이션이 존재하지 않음.
