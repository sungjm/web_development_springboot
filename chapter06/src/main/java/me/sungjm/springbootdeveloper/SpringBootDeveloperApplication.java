...
5장. '데이터베이스' 조작이 편해지는 'ORM'
    1. 데이터베이스(database)란?
        ...
        DBMS(DataBase Management System) : 기본적으로 데이터베이스는 많은 사람이 공유할 수 있어야 하고,
            ...
            관계형 DBMS
                ...
                RDBMS는 테ㅣ블('표'를 의미) 형태로 이루어진 데이터 저장소로, 예를 들어
            ...
            필수 용어
                ...
                3) 열(column) : 테이블의 구성 요소 중 하나이며, 행에 저장되는 유형의 데이터.
                    ...
                    이메일 열에 숫자가 들어가거나, 나이 열에 문자열이 들어갈 수 없기 띠ㅐ문에 데이터에 대한 무결성을 보장
                4) 기본키(Prime Key) : 행을 구분할 수 있는 식별자. 이 값은 테이블에서 유일해야 하며 "중복값을 가질 수 없음"
                    보통 데이터를 수정하거나 삭제하고, 조회할 때, 사용되며 다른 테이블과 관계를 맺어 데이터를 가져올 수 도 있음
                    또한 기본키의 값은 수정도어서는 안 되며 유효한 값이어야 함. 즉 ,NULL이 될 수 없음
                        -> 이전 수업에서 nullable = false
                5) 쿼리(Querry) : 데이터베이스에서 데이터를 조회하거나, 삭제, 생성, 수정과 같은 처리를 하기 위해 사용하는 명령문
                    SQL이라는 데이터베이스 전용 언어를 사용하여 작성함.

                    SQL(Structured Query Language) : 구조화된 정의 언어
    ...
    2. ORM이란?
        ...
        장점 :
            ...
            2) 객체지항적으로 코드를 작성할 수 있기 때문에 비지니스 로직에만 집중 가능
        ...
        단점 :
            ...
            2) 복잡하고 무거운 쿼리는 ORM으로 해결이 불가능한 경우가 있음.(JOIN이 여러번 들어가지거나 / 서브쿼리가 복잡한 경우)

    스프링 데이터 JPA에서 제공하는 매서드 사용
