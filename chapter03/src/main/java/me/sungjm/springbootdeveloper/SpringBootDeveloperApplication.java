...
/*
        ...
        스프링 부트에서의 계층
            1) 프레젠테이션(Presentation)
                HTTP 요청을 받고 이 요청을 비지니스 계층으로 전송하는 역할 -> Controller가
                    프레젠테이션 계층 역할
                TestController 클래스와 같은 것을 의미하여 스프링 부트 내에 여러 개가 있을 수 있음
            2) 비지니스(Business)
                모든 비지니스 로직을 처리
                    * 비지니스 로직 : 서비스를 만들기 위한 로직을 의미하며, 웹 사이트에서 벌어지는
                        모든 직업 -> 주문 서비스라고 한다면 주문 개수, 가격 등의 데이터를 처리하기
                        위한 로직, 주문 처리를 하다가 발생하는 예외 처리 로직, 주문을 받거나 취소하기
    ...
    스프링 부트 프로젝트 디렉토리 구성
        : 스프링 부트에는 정해진 구조가 없지만, 추천 프로젝트 구조가 있고, 이를 많은 개발자들이 따르도록
            ...
            main : 실제 코드를 작성하는 공간. 프로젝트 실행에 필요한 소스 코드나 리소스 파일은
                모두 이 폴더 안에 들어있음(프로젝트 생성시 자동 구현딤)


            test : 프로젝트의 소스 코드를 테스트할 목적의 코드나 리소스 파일이 들어있음
                (원래는 자동생성 되는데 어제 제가 만들었을 때 안만들어지는 걸로 보아 설정 따라
                다른듯합니다)
    ...
    2. 스프링 부트 3 프로젝트 발전시키기
        ...
        각 계층의 코드를 추가할 예정 -> 이를 통해 계층이 무엇이고, 스프링 부트에서는 계층을 어떻게
         나누는지를 감을 압아나가실 수 있도록 수업하겠습니다(목요일)
         ...
         implementation : 프로젝트 코드가 컴파일 시점과 런타임에 모두 해당 라이브러리르 필요로 할 때 사용
         ...
         annotationProcessor : 컴파일 시에 애너테이션을 처리할 때 사용되는 도구의 의존성 지정
    ...
    작동을 확인해보겠습니다.
        ...
        01 단계 -> resources 폴더에 sql문 하나 추가할 겁니다.
            resources 우클릭 -> new -> file -> data.sql

        02 단계 - 이제는 기존에 만들어둔 application.yml 파일을 수정할겁니다.

        03 단계 - 서버 실해 후에 ctrl + f 눌러서 create 검색해서 table이 생성됐는지 확인

        04 단계 - Postman에서 http 요청을 시도합니다.
            1) 포스트맨 실행.
            2) http 매서드를 get으로 설정하고 url애
