   ...
   사용자가 서버에 접근할 때 이 사용자가 인증된 사용자인지 확인하는 방법 중 하나로,
    ...
    토큰 기반 인증 :
        *토큰 : 서버에서 클라이언트를 구분하기 위한 유일한 값
        ...
        서버에 토큰을 생성해서 클라이언트에게 제공하면, 클라이언트는 이 토큰을 가지고 있다가
        ...
        4) 이후 인증이 필욯ㄴ API를 사용할 때 토큰을 함께 보냄 ->
        5) 서버는 토큰이 유효한지 검증
        6) 트콘이 유효하다면 클라이언트의 요청한 내용을 처리

    토큰 기반 인증의 특징
        1) 무상태성
            사용자의 인증 정보가 담겨있는 토큰이 서버가 아닌 클라이언트에 있으므로
                서버에 저장할 필요 X
        ...
        2) 확장성
            ...
            추가로 페이스북 로기은, 구글 로그인과 같이 토큰 기반 인증을 사용하는
            다른 시스템에 접근해 로그인 방식을 확장할 수 있고,
            이를 활용해 다른 서비스에 권한을 공유할 수도 있음.

        3) 무결성
            토큰 기반 인증 방식은 HMAC(Hash-based Message Authenticaation) 기법이라고도 하는데,
            토큰을 발급한 이후에는 토큰 정보를 변경하는 행위를 할 수 없음.
            만약 토큰을 한 글자라도 변경하면 서버에서는 유효하지 않은 토큰이라고 판단합니다.

    JWT(JSON WEB TOKEN) -> json형태로 주고받는것
        ...
        JWT 구조 : 온점(.)을 기준으로 헤더(header), 내용(payload), 서명(signature)로 이루어짐.
            ...
            2) 내용에는 토큰과 관련된 정보를 담습니다. 내용의 한 덩어리를 클레임(claim)
                ...
                등록된 클레임 / 공개 클레임 / 비공개 클레임으로 나뉨.
                    ...
                    등록/공개 클레임 제외 나머지느 전부 비공개 클레임에 해당
        ...
        2. JWT 서비스 구현
            의존성과 투큰 제공자를 추가하고 리프레시 토큰 도메인 토큰 필터를 구현하면
            -> JWT 서비스를 사용할 준비가 됐다고 볼 수 있습니다.
            bild.gradle로 이동해서 의존성 추가


            토큰 제공자를 추가할겁니다.
                jwt를 사용해서 JWT를 생성하고 유효한 토큰인지 검증하는 클래스 추가할 예정
                    JWT 토큰을 만들려면 이슈 발급자(issuer), 비밀키9secret_key)를 필수로 설정
