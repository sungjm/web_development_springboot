...
public class Article {
    ...
    // 이상의 update 메서드 정의 후에 -> 수정 요청과 관련된 DTO WKRTJD

    /*
        @Builder 애너테이션은 롬복에서 지원하는 애너테이션으로, 생성자 위에 입력하면 빌더 패턴
        방식으로 객체 생성 가능

        객체를 유연하고 직관적으로 생성할 수 있기 때문에, 게발자들이 애용하는 '디자인 패턴'
        어느 필드에 어떤 값이 들어가는지 명시적으로 파악할 수 있습니다.

        repository pacakage 생성(domain과 같은 라인에) -> BloRepository.java 인터페이스 생성
     */
}
