...
-- 예를 들어 실습 데이터에서 회원 수를 구하려면 어떡해야할까, 혹은 전체 회원 수가 아니라,
-- 국가별 회원 수를 구하는 방법, 제품별 매출을 구하는 방법 등을 실제로 알아볼 수 있음.

-- SQL내에서 사용하는 함수들은 주로 평군, 개수, 합계 등을 구하는 집계 함수,
-- 문자열을 원하는 만큼 잘라내거나 대/소문자를 변경하는 등의 기능을 수행하는 일반함수,
-- 등으로 나눌 수 있다.
...
-- 예를 들어 '국가별 회원 수를 구하라' 혹운 '일별 매출 계산하라'와 같은
-- 그룹화 기준을 지정해서 원하는 계산을 수행할 수 있음

-- GROUP BY 로 계산한 결과를 필터링하는 HAVING 명령어
-- GROUP BY로 적용해서 '국가별 회원 수를 구하고' 이후에 '그리고 회원 수가 10명 이상인'
...
select count(*) from users; 		-- count(컬럼명, 사실 *) 집계함수에 해당, 대상 테이블 전체 행의 개수를 세는 역할

-- users에서 존재하는 country 컬럼의 데이터 개수
select count(country) from users; 	-- country 컬럼에서 null이 아닌 개수, 즉 중복 계산

select * from users where country is null;

-- country의 데이터값의 종류를 계산하고 싶다면 어떡해야할까?
select count(distinct country) from users;		-- 중복을 제거하기 위해서는 distinct

-- distinct : 중복 값을 제거하여 고유한 값만 반환하는 키워드 /select와 함께 사용

select count(1) from users;			-- count(컬럼숫자) 가능
...
substr(컬럼명, 시작위치, 가져올문자개수) : 문자열을 지정한 시작 위치부터 지정한 문자개수만 가져옴
...
-- 집계 하수만으로 원하는 결과를 얻을 수 없을 때(데이터 전체가 아니라, 원하는 기준으로
...
-- 예시 : users에서 country가 korea인 회원수를 출력
...
-- 다음시간에 저희는 group by를 통해 어떻게 묶을지 학습하도록 하겠습니다.