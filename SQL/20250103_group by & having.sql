...
-- users애서 country별로 마케팅 수신 동의한 회원 수와 동의하지 않은 회원 수를 구해 출력할 것
...
-- 국가별로는 오름차순(default라서 미표기), uniqueusercnt를 기준으로는 내림차순으로 정렬
-- select절 - from 테이블명 - group by절 - order by 절  : 순서

group by에 두개 이상의 기준 컬럼을 추구하면 데이텉가 여러 그룹으로 나뉨.
...
따라서 '중요한 기준을 앞에 배치'해서 시각화오 그룹화에 대한 우선순위를 결정할 필요 있음.

-- group by 정리
-- 1. 주어진 컬럼을 기준으로 데이터를 그룹화하여 '집계함수'와 함께 사용
-- 2. group by 뒤에는 그룹화할 컬럼명을 입력, 그룹화한 컬럼에 집계 함수를 적용하여 그룹별 계산 수행.
-- 3. 형식 : group by 컬럼명1, 컬럼명2 ...
-- 4. group by에서 두 개 이상의 기준 컬럼을 조건으로 추가하여 여러 그룹으로 분할 가능한데,
-- 		컬럼 순서에 따라 결과에 영향을 미치므로 우선순위를 생각할 필요있음.
...
select order_id, sum(quantity) from orderdetails group by order_id order by sum(quantity) desc; 
...
select substr(order_date, 1, 7), count(distinct user_id) 
	from orders group by substr(order_date, 1, 7) order by substr(order_date, 1, 7) desc;

-- Having
-- group by를 이용해서 데이터를 그룹화하고, 해당 그룹별로 집계 연산을 수행하여,
-- 국가 별 회원 수를 도출해낼 수 있었습니다. (count())
-- 예를 들어서, 회원 수가 n명 이상인 국가의 회원 수만 보는 등의 조건을 걸러면 어떡해야 할까?

-- where절을 이용하는 방법이 있긴 핮만 추가적인 개녀에 대해서 학습할 예정입니다.
-- 언제나 where절을 쓰는 것이 용이하지 않다는 점 부터 짚고 넘어가서 having 학습을 할 예정

-- users에서 country가 korea, usa, france인 회원 숫자를 도출할 것.
select country, count(distinct id) from users 
	where country in ("korea", "usa", "france") group by country;

where을 통해서 원하는 국가만 먼저 필터링하고, group by룰 했습니다.
여기서 필터링 조건은 country 컬럼의 데이터 값에 해당합니다.

만약에 회원 수가 8명 이상인 국가의 회원 수만 필터링하려면 어떡할까?

-- select country, count(distinct id) from users where count(distinct id) >= 8; 오류 나는 예시

-- users에서 회원 수가 8명 이상인 country 별 회원 수 출력(회원 수 기준 내림차순)

-- select country, count(distinct id) from users group by country 
-- 	having count(distinct id) >= 8 order by count(distinct id) desc;

select country, count(distinct id) from users group by country 
	having count(distinct id) >= 8 order by 2 desc; 		-- 두 번째 칼럼을 내림차순 정리
...
1. where에서 필터링을 시도할 때 오류가 발생하는 이유 :
	where은 group by보다 먼저 실행되기 때문에 그룹화를 진행하기 전에 행을 필터링 함.
 	...
 	순서상으로 group by 보다 뒤에 있어야 해서 where절 도입이 불가능 함.

2. 즉 group by를 사용한 집계 값을 필터링할 때는 -> having을 사용.

-- orders에서 담다 직원별 주문 건수와 주문 회원 수를 계산하고, 주문 건수가 10건 이상이면서
-- 주문 회원 수가 40명 이하인 데이터만 출력(단, 주문 건수 기준으로 내림차순 정렬할 것. staff_id, user_id, id 컬럼 사용)

select staff_id, count(distinct id), count(distinct user_id) from orders
	group by staff_id having count(distinct id) >= 10 and count(distinct user_id) <= 40
...
-- 6. order by를 통해 특정 컬럼 및 연사 결과를 통한 오름차순 및 내림차순으로 나열함.

-- 연습문제
-- 1. orders에서 회원별 주문 건수 추출하라(단 주문 건수가 7건 이상인 회원의 정보만 추출, 
-- 주문 건수 기준으로 내림차순 정렬, user_id와 주문 아이디(id)컬럼 활용 할 것)

select user_id, count(distinct id) from orders group by user_id 
	having count(distinct id) >= 7 order by count(distinct id) desc;

-- 2. users에서 country 별 city 수와 국가별 회원(id) 수를 추출.
-- 	단 도시 수가 5개 이상이고 회원 수가 3명 이상인 정보만 추출하고,
-- 	도시 수, 회원 수 기준으로 모두 내림차순 정렬할 것

	select country, count(distinct city), count(distinct id) from users group by country 
...
-- 	(hint : 얘는 where도 있고 having도 있다.)
	
select country, count(distinct id) from users where country in ("usa", "brazil", "korea", "argentina", "mexico")
	group by country having count(distinct id) >= 5 order by count(distinct id) desc;

-- SQL 실무 상황에서의 GROUP BY & HAVING
-- 1. 2025-01-03에 음식 분류별(한식, 중식, 분식, ...) 주문 건수 집계

-- select 음식분류, count(distinct 주문 아이디) as 주문건수
-- 	from 주문정보
