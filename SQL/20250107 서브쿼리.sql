...
-- 이상의 쿼리의 문제점은 결합 기능을 지는 union의 결과값이 select * from users;와
-- 동일하다는 점입니다.
...
(select id, phone, city, country		-- 일부 컬럼만 지정해서 출력
	...
	from users)								-- 기준이 되는 첫 번째 select절에서 선택하는 
	order by id;							-- 컬럼의 종류 및 개수가 완벽히 일치해야 합니다.
	
-- 실무에서는 union all이 더 권장 됩니다. union의 경우 중복 제거가 있는데, 대량의 데이터를 대상으로
...
(select id, phone, city, country from users where country = "korea") 
	union all (select id, phone, city, country from users where country = "mexico") 
	order by country;

-- 연습문제
-- 1. orders에서 order_date가 2015년 10월 건가 2015년 12월인 건을 select로 각각 추출하고,
...
(select * from orders where order_date >= "2015-10-01" and order_date < "2015-11-01")
	union all (select * from orders where order_date >= "2015-12" and order_date < "2016-01-01") 
...
-- 다른 쿼리
(select * from orders where substr(order_date, 1, 7) = "2015-10") union all 
(select * from orders where substr(order_date, 1, 7) = "2015-12") order by order_date desc;
-- 2. users에서 usa에 거주 중이면서 마케팅 수신 동의(1)한 회원 정보와 france에 거주중이면서
...
(select id, phone, country, city, is_marketing_agree from users where is_marketing_agree = "1" and country = "usa")
	union all (select id, phone, country, city, is_marketing_agree from users where is_marketing_agree = "0" and country = "france")
	order by country desc;
-- 3. union을 활용하여 orderdetails와 products를 full outer join 조건으로 결합하여 출력하세요. -> 실무나 시험에서 사용되는 경우 거의 없음
(select * from orderdetails o left join products p on o.product_id = p.id) union 
...
-- 단 하나 이므로 select name, price, avg(price) from products;로 작성하면
-- 1행짜리만 도출됨.
...
-- 특정한 단일 결고값을 각 행에 적용을 하고 싶다면 이상과 같은 하드코딩이 가능합니다.
...
-- 스칼라 서브 쿼리를 작성할 때 '단일값'이 반환되도록 작성해야 한다는 점에 유의하세요.
-- 만약 2개 이상의 집계 값을 기존 테이블에 추가하여 출력하고 싶다면 스칼라 서브 쿼리를 따로
-- 나누어서 작성해야 합니다.

-- users에서 city별 회원 수를 카운트하고, 회원 수가 3명 이상인 도시 명과 히원 수를 출력해보자.
-- 단 회원 수를 기준으로 내림차순
select city, count(distinct id) from users group by city;		-- > 도시 별로 id 개수를 계산했습니다.

select city, count(distinct id) from users group by city 
	having count(distinct id) >= 3 order by count(distinct id) desc;

-- orders와 staff 활용해 last_name이 kyle이나 scott인 직원의 담당 주문을 출력하려면
-- 단 서브쿼리 형태를 활용하자

select id from staff where last_name in ("kyle", "scott");   		-- 조건절에 쓰일 경우 scalar 서브가 아니었다는 점에 주목

-- 이상의 코드는 staff 테이블에서 id 값이 3, 5를 도출해냈습니다.
-- 해당 결과를 가지고 orders 테이블에 적용하는 형태로 작성합니다.

select * from orders where staff_id in (		
select id from staff where last_name in ("kyle", "scott")) order by staff_id;

-- where절 내에서 필터링 조건 지정을 위해 중첩된 서브 쿼리를 작성 가능
-- where에서 in 연산자와 함계 서브 쿼리를 활용할 경우 :
-- 컬럼 개수와 필터링 적용 대상 컬럼의 개수가 '일치'해야만 합니다.

-- 이상의 코드에서 서브쿼리의 결과 도출되는 컬럼의 숫자 = 1 (staff테이블의 id) / 행 = 2

select * from orders where (staff_id, user_id) in (		-- 필터링 대상 컬럼 개수 = 2 / 서브 쿼리 컬럼 개수 = 2
select id, user_id from staff where last_name in ("kyle", "scott")) order by staff_id;

-- 결과값으로 직원 정보 테이블에 존재하는 id, user_id와 동일한 값이 orders 테이블의
-- staff_id, user_id 컬럼에 있을 경우 반환하며 출력합니다.
...
-- 1) 과 2)의 inner join rnnas -> from 절에서 이루어집니다.
select * from 
(select * from orders where order_date >= "2015-07-01" and order_date < "2015-08-01") o  -- 1)의 결과가 테이블이었기 때문에
 inner join (select * from orderdetails where quantity >= 50) od on o.id = od.order_id;
...
-- from절에서 사용되는 서브 쿼리 : '인라인 뷰', 마치 테이블처럼 서브 쿼리의 결과값을 사용 가능.
...
-- 특징 :
-- in 연산자의 경우에 다중 컬럼 비교를 할 때는 서브 쿼리에서 추출하는 컬럼의 개수와
...
select substr(created_at, 1, 4), count(distinct id) from users 
...
select country, city, sum(is_auth) -- is_auth가 1이면 본인인증한거니까 is_auth들의 합이 10 -> 본인인증한 회원 숫자
from users 
group by country, city order by sum(is_auth) desc;